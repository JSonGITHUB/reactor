...
const Scheduler = () => {
    ...
    // UI
    return (
        <div className='containerDetail mt--30' ref={scheduleRef}>
            <div className='containerBox contentLeft color-yellow'>
                🕒 Scheduler
            </div>
            <div className='containerBox bg-lite bottomBorderMedium'>
                <textarea
                    className='containerDetail p-20 bg-dark width--10 ml-5 mr-5 mt-5 mb-5'
                    rows={10}
                ...
                />
                <div className='containerDetail color-orange m-5 pt-10 pb-10 size12 pl-15 pr-15 contentLeft lh-15'>
                ...
                <div className='flexContainer'>
                    <div className='containerBox button bg-green flex2Column' onClick={handleParse}>
                        Submit Schedule
                    </div>
                </div>
            </div>
            {tasks.map((task) => (
                <div
                    ...
                    className={`containerBox ${task.isCompleted ? 'bg-dkGreen' : task.isActive ? 'bg-silver' : task.paused ? 'bg-dkYellow' : 'bg-lite'}`}
                >
                    ...
                    <div className='containerBox contentLeft color-yellow'>
                        {task.editingField === 'title' ? (
                            ...
                            <span onClick={() => handleFieldEdit(task.id, 'title')} className='button'>
                                {task.title}
                            </span>
                        )}
                    </div>
                    <div className='containerBox contentLeft'>
                        {task.editingField === 'duration' ? (
                            ...
                            <span onClick={() => handleFieldEdit(task.id, 'duration')} className='button'>
                                Time: {formatTime(task.remaining)}
                            </span>
                        )}
                    </div>
                    <div className='containerBox contentLeft'>
                    ...
                    {/* Timer controls */}
                    <div className='containerBox flexContainer bg-mediumDark'>
                        {!task.isCompleted && (
                            <>
                                {
                                    (!task.isActive)
                                    ? <div title='start' className='flex4Column' onClick={() => startTask(task.id)} disabled={task.isActive}>
                                        <div className='button size40 mt-10 mb-10'>
                                            ▶️
                                        </div>
                                    </div>
                        ...
                        {task.isCompleted && <span className='button' onClick={() => unCheckTask(task.id)} style={{ color: 'green' }}>✅ Completed</span>}
                    </div>
                </div>
            ))}
        </div>
