...
const DoseContent = () => {
    ...
    const displayAddForm = () => <div className=''>
        ...
        <div className='flexContainer containerBox bg-lite'>
            ...
            {
                <div className='containerBox  flex2Column bg-dkYellow button' onClick={(edit) ? cancelEdit : () => setFormCollapsed(true)}>
                    Cancel
                </div>
            }
        </div>
            ...
            <div className='mt--30'>
                ...
                <div className='containerBox' ref={scheduleRef}>
                {
                    ...
                    ? Object.entries(grouped()).map(([date, times], i) => (
                        <div key={getKey(i)}>
                            <div key={getKey(i)} className={`containerDetail p-10 m-5 bg-${(date == today) ? 'green color-white' : 'yellow'}`}>
                            <CollapseToggleButton
                                title={<div className={`${(date == today) ? 'color-white' : 'color-dark'} size30`}>{date}</div>}
