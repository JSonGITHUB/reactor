...
const Header = ({ company, width, isMotionOn, isSignedIn, setSignIn }) => {
    ...
    const hamburgerOpen = <div className=''>
        <div className='flexContainer width-100-percent'>
            <div className='flex3Column contentLeft'>{logoButton(mobileLogo)}</div>
            <div title='notifications' className='flex3Column button p-15 centerVertical' onClick={()=>setNoticationCollapse(!notificationCollapse)}>👀<span className='size15 '>{notifications.length}</span></div>
            <div className='flex3Column contentRight'>{getMenuButton}</div>
        </div>
    ...
    const hamburgerClosed = <div>
        <div className='flexContainer width-100-percent'>
            ...
            <div title='notifications' className='flex3Column button p-15 centerVertical' onClick={() => setNoticationCollapse(!notificationCollapse)}>
                👀
                <span className='size15'>
                    {notifications.length}
                </span>
            </div>
            <div className='flex3Column contentRight'>{burgerButton}</div>
        </div>
        {
        /* 
    ...
    return (
        <div className='App-header'>
            ...
            <Branding />
            {
                ...
                ? null
                    : <div className='t-0 fixed mt-50 containerDetail p-10 mt--20 width--20 flexContainer bg-dark h-scroll'>
                        <input
                            id='app search'
                            name='app search'
