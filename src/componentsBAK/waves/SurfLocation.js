                ...
                <div className='flexContainer'>
                    <div className='flexContainer m-auto'>
                        ...
                        <div className='columnLeft'>
                            <div className='mt-5'>{item.swell.map((swell, i) => <span key={getKey('swell')} className={`mb-5 ${(swell === status.swell1Direction) ? statusClass(swell1Match(item)) : subStatusClass(swell2DirectionMatch(swell))}`}>{swell}{((i+1) === item.swell.length)? '' : ', '}</span>)}</div>
                            <div className='mt-5'>
